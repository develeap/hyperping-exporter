...
func TestBuildUserAgent(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			version:         "dev",
			appendUserAgent: "",
			expectedPattern: "hyperping-exporter/dev",
			shouldContain: []string{
...
func TestBuildUserAgent_Format(t *testing.T) {
	...
	// Should match pattern: hyperping-exporter/VERSION (goVERSION; OS/ARCH)
	expectedPrefix := "hyperping-exporter/1.2.3 ("
...
func TestClient_DefaultVersion(t *testing.T) {
	...
	if !strings.Contains(receivedUserAgent, "hyperping-exporter/dev") {
		t.Errorf("Default User-Agent should contain 'hyperping-exporter/dev', got %q", receivedUserAgent)
	}
}
...
func TestClient_UserAgentOnRetry(t *testing.T) {
	...
	for i, ua := range receivedUserAgents {
		...
		if !strings.Contains(ua, "hyperping-exporter/1.0.0") {
...
func TestWithVersion_Option(t *testing.T) {
	...
	expectedUA := fmt.Sprintf("hyperping-exporter/5.4.3 (%s; %s/%s)",
		runtime.Version(),
		runtime.GOOS,
		runtime.GOARCH,
	)
